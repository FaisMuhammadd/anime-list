...
const Page = async ({ params: { id } }) => {
  ...
  const collection = await prisma.collection.findFirst({
    where: { user_email: user?.mail, anime_mal_id: id }
  })
  ...
  return (
    <>
      ...
      <div className="pt-4 px-4 flex gap-2 text-color-primary overflow-x-auto">
        <div className="w-36 flex flex-col justfy-center items-center rounded border border-color-primary p-2">
          <h3>PERINGKAT</h3>
          <p>{anime.data.rank}</p>
        </div>
        <div className="w-36 flex flex-col justfy-center items-center rounded border border-color-primary p-2">
      ...
      <div className="p-4">
        ...
        {
          user && <CommentInput anime_title={anime.data.title} anime_mal_id={id} user_email={user?.email} username={user?.name}/>
        }
      </div>
      <div>
